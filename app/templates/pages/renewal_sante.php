...
<p>Étant mineur·e, la réglementation fédérale exige un questionnaire de santé chaque saison.</p>

<?php if ($attestation !== null): ?>
    <div class="alert alert-ok">✔ Cette étape est déjà complétée cette saison
        (<?= $attestation['outcome'] === 'all_negative' ? 'attestation signée' : 'certificat médical fourni' ?>).
        Vous pouvez la refaire ci-dessous ou <a href="/espace/renouvellement/representant-legal">continuer</a>.</div>
<?php endif; ?>
...
<ol>
    <li>Faites renseigner (avec l'aide de votre représentant légal si besoin) le
...
<form method="post" enctype="multipart/form-data" class="form form-wide" id="sante-form">
    ...
    <fieldset id="attestation-block" hidden>
        ...
        <label class="choice"><input type="checkbox" name="read_questionnaire" value="1">
            J'atteste que <?= htmlspecialchars($bjUser['firstname'] ?? '', ENT_QUOTES) ?> a renseigné le questionnaire de santé
        ...
        <div class="grid2">
            <div><label for="guardian_fullname">Représentant légal (nom, prénom) *</label>
                <input id="guardian_fullname" name="guardian_fullname" maxlength="100"
                       value="<?= htmlspecialchars((string) ($old['guardian_fullname'] ?? ($attestation['guardian_fullname'] ?? '')), ENT_QUOTES) ?>"></div>
            <div><label for="place">Fait à *</label>
                <input id="place" name="place" maxlength="100" value="<?= htmlspecialchars((string) ($old['place'] ?? ''), ENT_QUOTES) ?>"></div>
        </div>
